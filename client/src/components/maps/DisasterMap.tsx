...
const DisasterMap: React.FC<DisasterMapProps> = ({
  ...
  const getMarkerColor = (severity: Alert['severity']) => {
    switch (severity) {
  ...
  const createCustomIcon = (alert: Alert) => {
  ...
  const getAlertEmoji = (type: Alert['type']) => {
    switch (type) {
  ...
  const getRadiusColor = (severity: Alert['severity']) => {
    switch (severity) {
